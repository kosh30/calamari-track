...
- **Stempelungen des Plugins haben kein Projekt.** `clockIn` nimmt keine Argumente, also auch kein Projekt. In der Zeiterfassung erscheinen sie als „Ohne Projekt“ (WITHOUT A PROJECT), während Web und Handy das Projekt „Check-in“ setzen (beobachtet 2026-09-22). Wenn Auswertungen oder Regeln der Firma „Check-in“ verlangen, ist das die größte Lücke des MCP-Wegs. Ändern lässt es sich nur im Web: Laut Calamari-Hilfe kann man in der Stempeluhr das Projekt wechseln, ohne die Uhr anzuhalten. Auf MCP-Seite bräuchte `clockIn` einen Projekt-Parameter.
