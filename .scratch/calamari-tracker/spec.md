...
- **Vertrag des Helfers:** Unterbefehle `login`, `whoami`, `status`, `start-time [--after HH:MM]`, `end-time --after HH:MM`, `clock-in`, `clock-out`, `day-info --date YYYY-MM-DD`. Die Ausgabe ist immer genau ein JSON-Objekt auf stdout. Bei Erfolg `{"ok": true, ...}`, bei Fehlern `{"ok": false, "error": {"code": ..., "message": ...}}` mit den Codes u.a. `AUTH_REQUIRED`, `NETWORK`, `RATE_LIMITED`, `MCP_ERROR`. Der Exit-Code ist ungleich 0 bei Fehlern.
