...
- **Vertrag des Helfers:** Unterbefehle `login`, `whoami`, `tools` (die Tool-Liste des MCP-Servers, Vergleich mit `docs/mcp-tools.json` siehe ADR 0001), `status`, `start-time [--after HH:MM]`, `end-time --after HH:MM`, `clock-in`, `clock-out`, `day-info --date YYYY-MM-DD`. Die Ausgabe ist immer genau ein JSON-Objekt auf stdout. Bei Erfolg `{"ok": true, ...}`, bei Fehlern `{"ok": false, "error": {"code": ..., "message": ...}}` mit den Codes u.a. `AUTH_REQUIRED`, `NETWORK`, `RATE_LIMITED`, `MCP_ERROR`. Der Exit-Code ist ungleich 0 bei Fehlern.
...
- **Letzte Warnung und Auto-Abschluss:**
  ...
  - „+1 h“ und eine verschickte letzte Warnung gehören zu der Schicht, in der sie entstanden. Eine danach begonnene Schicht (z.B. nach einem Auto-Abschluss wieder eingestempelt) bekommt ihre eigene Warnung. Solange der Beginn einer im Web begonnenen Schicht noch gesucht wird, gibt es weder Warnung noch Auto-Abschluss.
...
- **Gesamtzeit heute (beobachtet):**
  ...
  - Grenzen, deshalb nennt das Panel die Summe „beobachtet“:
    - Schichten, die begannen und endeten, während das Plugin aus war, fehlen, ebenso eine Schicht, deren Beginn nie gefunden wurde.
    - Wer zwischen zwei Abfragen im Web aus- und wieder einstempelt, dem wird die Lücke als Arbeitszeit gezählt. Die Summe kann dann also auch zu hoch sein.
    - Die Schicht vom Vortag zählt nicht.
- **Pause:**
...
- **Standardwerte der Config:**
  ...
  Gepflegt werden die Werte über das Einstellungsschema des Bar-Widgets im Plugin-Manifest. Die installierte omarchy-shell zeigt dieses Schema nicht grafisch an. Deshalb bringt das Plugin eine eigene Einstellungsseite mit: Rechtsklick auf das Bar-Icon führt zu „Einstellungen“, einem Formular aus dem Schema (`SettingsForm.qml`, Prüfung in `js/settingsform.mjs`, das Format der Textfelder steht als `format` im Schema). Gespeichert wird über `updateEntryInline` in den Bar-Eintrag von `shell.json`, wie mit `omarchy bar set kosh.calamari-tracker <key> <wert> --json`.
